...
<meta property="og:description" content="information of car manager system" />
				...
				<form action ="${context}/login" class="form-horizontal" method = "POST" modelAttribute = "company" >
					<sec:csrfInput />
						<c:if test="${!empty sessionScope['SPRING_SECURITY_LAST_EXCEPTION']}">
		        			<div class="alert alert-danger" style="width:20% ;margin:20px auto" >
		        				Invalid company name or password
		        			</div>
						</c:if>
					<div class="input-group" style = "margin-left:40% ; margin-bottom:30px">
						<label class="input-group-addon" for ="name"><i class="fa fa-user"></i></label>
						<input style="width:31.200%" type="text" class = "form-control" id = "name" name="name" placeholder = "Username" value="${sessionScope['SPRING_SECURITY_LAST_USERNAME']}" required />
					</div>

					<div class="input-group" style = "margin-left:40%; margin-bottom:30px">
						<label for="password" class="input-group-addon"><i class="fa fa-lock"></i></label>
						<input style="width:31.200%" type="password" class="form-control"  id="password" name = "password" placeholder="Password" required />
					</div>
	
					<div class="form-group" style = "position:relative ;margin-left: 14.25%">
						<div class="col-sm-10">
							<button type="submit" class="btn btn-success" style= "width:100px">Login</button>	
							<button type="reset" class="btn btn-success" style = "margin-left:50px ; width:100px">Reset</button>
						</div>
					</div>
				</form>
				<div class="spacer"></div>
